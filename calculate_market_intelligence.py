...
if len(all_dates)<302:
    raise SystemExit(f'Need at least 302 raw sessions for full 252-session lookback; found {len(all_dates)}')
if len(dates)!=50:
...
# Require the full lookback for each EMA. This prevents a stock with only
# a short history from being treated as if it had a valid 50/200 EMA.
ema20=close_p.ewm(span=20,adjust=False,min_periods=20).mean()
ema50=close_p.ewm(span=50,adjust=False,min_periods=50).mean()
ema200=close_p.ewm(span=200,adjust=False,min_periods=200).mean()

# A true 52-week trading-session high requires 252 completed observations.
high252=close_p.rolling(252,min_periods=252).max()
...
for i,d in enumerate(dates):
    ...
    # NSE historical A/D methodology is based on average price.
    ...
    # Cap breadth uses the same NSE average-price advance/decline classification.
    ...
    if rk is not None:
        ...
        for lab,mask in [
            ...
            cap_valid=mask & breadth_valid
            if cap_valid.sum():
                val=round(float((breadth_ret[cap_valid]>0).mean()*100),2)
    ...
    h=high252.loc[d]
    nm=px.notna()&h.notna()
    near=int((px[nm]>=.9*h[nm]).sum()) if nm.any() else None
    near_pct=round(near/(nm.sum())*100,2) if near is not None and nm.sum() else None
    ...
    rows.append({
        ...
        'ema50_universe':int(m50.sum()),
        'ema200_universe':int(m200.sum()),
        'above_20_ema':a20,
        'above_50_ema':a50,
        'above_200_ema':a200,
        ...
        'near_10pct_52w_high_pct':near_pct,
...
roll55=close_p.shift(1).rolling(55,min_periods=55).max()
breakout=close_p>roll55
future2=close_p.shift(-2)

for r,d in zip(rows,dates):
    pos=int(np.searchsorted(all_dates,d))
    if pos<55 or pos+2>=len(all_dates):
        continue
    ev=breakout.iloc[pos-55:pos]
    fut=future2.iloc[pos-55:pos]
    cur=close_p.iloc[pos-55:pos]
    denom=int(ev.to_numpy().sum())
    numer=int((ev&(fut>=cur)).to_numpy().sum())
    if denom:
        r['breakout_follow_through_pct']=round(numer/denom*100,2)
...
if len(rows)!=50:
    raise SystemExit(f'Expected exactly 50 calculated sessions, got {len(rows)}')
if sum(m is not None for m in mcaps.values()) != 50:
    raise SystemExit('Official NSE market-cap coverage incomplete for the 50 output sessions')
if sum(bool(x) for x in ix.values()) != 50:
    raise SystemExit('Official NSE index-close coverage incomplete for the 50 output sessions')
if len(vx) < 50:
    raise SystemExit(f'Official NSE India VIX coverage incomplete: {len(vx)}/50')
