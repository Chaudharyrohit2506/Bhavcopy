import json,html,os
D=json.load(open('data/market_intelligence.json',encoding='utf8'))
rows=D['sessions']; latest=rows[-1]; dates=[r['session_date'] for r in rows]
...
css = r""":root{--bg:#060a12;--s:#0d1524;--b:#1e3150;--t:#f8fafc;--m:#94a3b8;--blue:#38bdf8;--green:#10b981;--pink:#f43f5e;--amber:#f59e0b;--purple:#c084fc}
...
"""
html_doc=html_doc.replace("__CSS__",css)

vals={'__DATE__':latest['session_date'],'__ADV__':f'{latest["advances"]:,}','__DEC__':f'{latest["declines"]:,}',
'__NET__':f'{latest["advances"]-latest["declines"]:+,}','__ADR__':n(latest['ad_ratio']),'__TRIN_KPI__':n(latest['trin']),
'__FT__':p(latest['breakout_follow_through_pct']),'__NEAR__':f'{latest["near_10pct_52w_high"]:,}','__NEARPCT__':p(latest["near_10pct_52w_high_pct"]),
'__LARGE__':p(latest['large_cap_adv_pct']),'__LARGEW__':p(latest['large_cap_adv_pct']),
'__MID__':p(latest['mid_cap_adv_pct']),'__MIDW__':p(latest['mid_cap_adv_pct']),
'__SMALL__':p(latest['small_cap_adv_pct']),'__SMALLW__':p(latest['small_cap_adv_pct']),
'__MCAP__':str(D['data_coverage']['mcap_files']),'__INDEX__':str(D['data_coverage']['index_files']),
'__VIX__':str(D['data_coverage']['vix_rows']),'__SECTOR40__':sector_block(D.get('sector_40d_ranking',[]),'return_40d_pct'),
'__SECTOR55__':sector_block(D.get('sector_55d_high_ranking',[]),'penetration_pct'),'__RVOL__':rv,'__LEDGER__':ledger,
'__DATES__':json.dumps(dates),'__AD__':json.dumps([r['ad_ratio'] for r in rows]),'__TRIN__':json.dumps([r['trin'] for r in rows]),
'__P20__':json.dumps([r['above_20_ema'] for r in rows]),'__P50__':json.dumps([r['above_50_ema'] for r in rows]),
'__P200__':json.dumps([r['above_200_ema'] for r in rows]),'__L__':json.dumps([r['large_cap_adv_pct'] for r in rows]),
'__M__':json.dumps([r['mid_cap_adv_pct'] for r in rows]),'__S__':json.dumps([r['small_cap_adv_pct'] for r in rows])}
for k,v in vals.items(): html_doc=html_doc.replace(k,v)
os.makedirs('output',exist_ok=True);open('output/NSE_All_Share_Institutional_Terminal.html','w',encoding='utf8').write(html_doc)
